...
// Datos de Red
const char* ssid = "INFINITY";  // Pon aquí el nombre de la red a la que deseas conectarte
const char* password = "contraseña";  // Escribe la contraseña de dicha red
...
int ledVerde = 13; // Led externo conectado a la placa
...
void setup() {
  ...
  pinMode (ledVerde, OUTPUT); // Se configura el pin como salida
...
void loop() {
  ...
  if ((timeNow - timeLast > wait) && statusLed == 0){// Comprobar el encendido del flash
    ...
    digitalWrite (ledVerde, HIGH);// Encender el flash
  ...
  if ((timeNow - timeLast > wait) && statusLed == 1){
    ...
    digitalWrite (ledVerde, LOW);// Apagar Led el flash
